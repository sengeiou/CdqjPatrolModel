 ...
 * desc：上报文件Adapter
 */
public class OrderSuperviseReportAdapter extends BaseQuickAdapter<OrderSuperviseReportBean, PatrolViewHolder> {

    private Context mContext;
    private ArrayList<ImageViewBean> mThumbViewInfoList;
    private String address, type;

    public OrderSuperviseReportAdapter(int layoutResId, @Nullable List<OrderSuperviseReportBean> data, Context context, String address, String type) {
    ...
    protected void convert(PatrolViewHolder helper, final OrderSuperviseReportBean item) {
        ...
        helper.setText(R.id.supervise_report_type, StringUtils.isTrimEmpty(type)?
                mContext.getString(R.string.null_text):type);
        helper.setText(R.id.supervise_report_address, StringUtils.isTrimEmpty(address)?
                mContext.getString(R.string.null_text):address);
