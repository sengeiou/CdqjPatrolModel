...
import com.cdqj.cdqjpatrolandroid.gis.bean.MetaData;
...
public class MBTilesLayer {
    ...
    public MBTilesLayer(String path) {
        try {
            ...
            LogUtils.d("MBTilesLayer", ex.getMessage());
            throw (ex);
        }
        if (_metadata == null) {
            LogUtils.d("MBTilesLayer---初始化GIS参数失败!");
        } else {
            LogUtils.d("MBTilesLayer---MetaData----->>>>>>" + _metadata.toString());
